...
public class CourseSchedule {
    ...
    public void removeCourseOfferByName(String name){
        for(int i=0;i<schedule.size();i++){
            if(schedule.get(i).getCourse().getName().equals(name)){
                schedule.remove(i);
            }
        }
    }
    
    public void addCourseOffer(Course c, FacultyProfile faculty, int seats){
        CourseOffer co = new CourseOffer(c);
        co.AssignAsTeacher(faculty);
